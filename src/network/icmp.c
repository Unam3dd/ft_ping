...
static int show_reply(iphdr_t *ip, const char *raw, rtt_t *rtt, int icmplen)
{
    ...
    memcpy(&pkt, raw,
           (size_t)icmplen < sizeof(pkt) ? (size_t)icmplen : sizeof(pkt));
    ...
    printf("%d bytes from %s: icmp_seq=%u ttl=%d time=%.3f ms\n", icmplen, src,
           (unsigned)ntohs(pkt.h.un.echo.sequence), ip->ttl, ms);
    return (0);
}
...
int send_icmp_echo(context_t *ctx, const fd_t fd, const sin_t *dst)
{
    ...
    bytes = sendto(fd, &pkt, sizeof(icmp_pkt_t), 0, (struct sockaddr *)dst,
                   sizeof(sin_t));
